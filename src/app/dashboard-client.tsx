...
function RecentCases({ allUsers, initialCases }: { allUsers: User[] | undefined, initialCases: Case[] }) {
...
function RecentActivity({ allUsers, initialCases, initialTasks, initialMeetings, initialAuditLogs }: { allUsers: User[] | undefined, initialCases: Case[], initialTasks: Task[], initialMeetings: Meeting[], initialAuditLogs: AuditLog[] }) {
    const router = useRouter();
    const [isOpen, setIsOpen] = useState(true);

    const activities = useMemo(() => {
        if (!initialCases || !initialTasks || !initialMeetings || !initialAuditLogs || !allUsers) return [];
        const twoWeeksAgo = subDays(new Date(), 14);

        const caseActivities = initialCases
            .filter(c => isAfter(new Date(c.createdAt), twoWeeksAgo))
            .map(c => ({
                id: `case-${c.id}`,
                type: 'case',
        ...
        const taskActivities = initialTasks
        ...
        const meetingActivities = initialMeetings
        ...
        const auditActivities = (initialAuditLogs || [])
    ...
    }, [initialCases, initialTasks, initialMeetings, allUsers, initialAuditLogs]);
    
    const isLoading = !allUsers;
...
export function DashboardClient({ stats, initialCases, initialTasks, initialMeetings, initialAuditLogs, allUsers }: DashboardClientProps) {
    const { user } = useAuth();
    const router = useRouter();
